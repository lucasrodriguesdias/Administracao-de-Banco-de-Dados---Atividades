...
-- Jefferson Luis Caetano - 12119456

-- Questão 1 (Criação do Banco e Inserção dos dados)
...
CREATE OR REPLACE VIEW View3_4 AS
...
CREATE OR REPLACE VIEW View3_6 AS
...
CREATE OR REPLACE VIEW View3_8 AS
...
CREATE OR REPLACE VIEW View3_10 AS
...
CREATE OR REPLACE VIEW View3_12 AS
...
SELECT * FROM View3_12;

-- Numero 13 

CREATE OR REPLACE VIEW View3_13 AS
...
SELECT * FROM View3_13;

-- Numero 14 

CREATE OR REPLACE VIEW View3_14 AS
SELECT fabricante,modelo,porta_mala_L FROM carros C
JOIN dimensoes D
ON D.fk_idCarro = C.idCarro
WHERE C.categoria = 'Hatch';
SELECT * FROM View3_14;
...
DROP TRIGGER IF EXISTS CHECK_DEVOLUCAO;
DELIMITER $$
...
DECLARE quilometragemInicial INT;
DECLARE quilometragemExcedente INT;
DECLARE diariaOriginal DOUBLE;

SET quilometragemInicial = (SELECT quilometragem 
							FROM LOCACAO L
							JOIN CARROS C
...
SET diariaOriginal =(SELECT valorDiaria 
					FROM LOCACAO 
					WHERE idLocacao = NEW.fk_idLocacao
					);

SET quilometragemExcedente = NEW.kmDevolvida - quilometragemInicial;

IF (quilometragemExcedente > 1000) THEN
SET NEW.valorDiariaAjustada = diariaOriginal + (diariaOriginal * 0.30);

ELSE
SET NEW.valorDiariaAjustada = diariaOriginal;
END IF;
...
SELECT * FROM DEVOLUCAO WHERE fk_idLocacao = 1;
...
CREATE INDEX carros_modelo ON carros(modelo ASC);
CREATE INDEX clientes_cpf ON clientes(cpf ASC);
CREATE INDEX devolucao_fk_idLocacao ON devolucao(fk_idLocacao ASC);
CREATE INDEX dimensoes_peso ON dimensoes(peso_kg ASC);
CREATE INDEX locacao_dataLocacao ON locacao(dataLocacao ASC);
CREATE INDEX log_locacao_dataLocacao ON log(dataLocacao ASC);
